...
const App: React.FC = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      // Using a known public cobalt instance (may change, but these are common)
      // Note: In a production environment, you would use your own backend proxy
      ...
      for (const instance of COBALT_INSTANCES) {
        try {
          ...
          const res = await fetch(instance, {
            ...
            body: JSON.stringify({
              url: url,
              videoQuality: qualityMap[quality],
      ...
      if (!response || response.status === 'error') {
        throw new Error(response?.text || 'All conversion servers are currently busy. Please try again later.');
      }

      setProgress(80);

      // Successfully got a response from Cobalt
      // Note: Cobalt might return a direct link (status 'stream') or a picker (status 'picker')
      if (response.status === 'stream' || response.status === 'redirect' || response.url) {
        setVideoInfo({
          title: response.filename || 'Your Download',
      ...
      <main className="max-w-4xl mx-auto pt-16 pb-24 px-4 sm:px-6">
        ...
        >
          ...
          <div className="absolute top-3 right-6">
            <span className="text-[10px] font-mono text-gray-700 uppercase tracking-widest">Axiom Engine</span>
          </div>
                    ...
                    <div className="flex-1 text-center sm:text-left">
                      ...
                      <div className="flex gap-2">
                        <a 
                          ...
                          className="flex-1 bg-white text-black hover:bg-gray-200 font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2 text-center"
                        >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            <div className="text-right">
              ...
              <p className="text-[10px] text-gray-700 mt-1 font-mono uppercase tracking-widest">
                Made by Axiom
              </p>
            </div>
          </div>
